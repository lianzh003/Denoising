...
class VisualizationAnalysis(object):
    ...
    @staticmethod
    def save_single_image(image_numpy, saved_image_path, min_v=0, max_v=2, cmap='jet'):
    # def save_single_image(image_numpy, saved_image_path, cmap='jet'):
        # plt.figure()
        plt.imshow(image_numpy, cmap=cmap, vmin=min_v, vmax=max_v)
        # plt.imshow(image_numpy, cmap=cmap)
        plt.axis('off')
        plt.colorbar()
        plt.savefig(saved_image_path, bbox_inches='tight')
        plt.close()

    def save_image(self, image_numpy, image_path, min_v=0, max_v=2, cmap='jet'):
    # def save_image(self, image_numpy, image_path, cmap='jet'):
        if len(image_numpy.shape) == 2:
            saved_image_path = image_path + '.png'
            self.save_single_image(image_numpy, saved_image_path, min_v, max_v, cmap)
            # self.save_single_image(image_numpy, saved_image_path, cmap)
        else:
            ...
            # self.save_single_image(image_numpy[:, :, int(d / 2)], saved_image_path, cmap)
            ...
            # self.save_single_image(image_numpy[:, int(w / 2), :].transpose((1, 0)), saved_image_path, cmap)
            ...
            # self.save_single_image(image_numpy[int(h / 2), :, :].transpose((1, 0)), saved_image_path, cmap)
